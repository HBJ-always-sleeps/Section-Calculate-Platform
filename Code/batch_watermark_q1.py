...
def add_watermark(pil_im, date_obj, weekday_str, hours, minutes):
    ...
    icon_width = 40   # 放大两倍: 20 -> 40
    line_spacing = 40  # 放大两倍: 20 -> 40
    icon_company_spacing = 16  # 放大两倍: 8 -> 16
    ...
    circle_radius = 12  # 放大两倍: 6 -> 12
    draw.ellipse([icon_x - circle_radius, icon_y - circle_radius - 8, 
                  icon_x + circle_radius, icon_y + circle_radius - 8], 
                 fill=(255, 0, 0), outline=(255, 255, 255), width=4)  # 线宽也放大
    
    triangle_points = [
        (icon_x, icon_y + circle_radius + 12),  # 放大两倍: 6 -> 12
        (icon_x - 8, icon_y - 4),  # 放大两倍: 4 -> 8, 2 -> 4
        (icon_x + 8, icon_y - 4)
    ]
...
def process_month(month_dir_name):
    """处理某个月份的所有图片"""
    if month_dir_name not in MONTH_CONFIG:
        print(f"跳过未知目录: {month_dir_name}")
        return 0
    
    date_obj, weekday_str = MONTH_CONFIG[month_dir_name]
    input_dir = BASE_DIR / month_dir_name
    output_dir = BASE_DIR / f"{month_dir_name}_已加水印"
    
    # 直接创建目录，不删除（文件会被覆盖）
    output_dir.mkdir(exist_ok=True)
    
    print(f"\n{'='*50}")
    print(f"处理目录: {month_dir_name}")
...
def main():
    ...
    total = 0
    for month_name in ["1月", "2月", "3月"]:
        count = process_month(month_name)
        total += count
