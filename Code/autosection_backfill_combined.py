# -*- coding: utf-8 -*-
# autosection_backfill_combined.py - 分层算量与回淤计算合并脚本
"""
合并 autosection 和 backfill 模块，在同一次运行中完成：
1. 分层算量：计算各地层在目标高程以下的面积
2. 回淤计算：计算DMX与设计断面线之间的回淤面积

测试完成后可迁移回 engine_cad_v3.py
"""
...
def run_autosection_backfill_combined(params, LOG):
    """分层算量与回淤计算合并任务
    
    在同一次运行中完成：
    1. 分层算量：计算各地层在目标高程以下的面积（基于更新断面线）
    ...
    try:
        ...
        LOG(f"[INFO] DMX图层（原始断面线）: {dmx_layer}")
        ...
        for input_path in file_list:
            ...
            for idx, dmx_data in enumerate(dmx_list):
                ...
                # 计算地层面积
                if below_layer_open is not None and not below_layer_open.is_empty:
                    # 绘制高程线（仅在有目标高程时绘制）
            ...
            # 生成Excel
            if combined_results:
                output_xlsx = os.path.join(output_dxf_dir, f"{base_name}_分层回淤合并_{timestamp}.xlsx")
                ...
                with pd.ExcelWriter(output_xlsx, engine='openpyxl') as writer:
                    ...
                    total_data = {
                        '统计项': [
                            '总断面数', 
                            f'{target_elevation}m以下总面积' if target_elevation else '开挖总面积',
                            '总回淤面积'
                        ],
                        '数值': [
                ...
                if target_elevation:
                    LOG(f"   {target_elevation}m以下总面积: {df_section['总面积'].sum():.3f} ㎡")
                else:
...
def main():
    ...
    if len(sys.argv) < 2:
        print("用法: python autosection_backfill_combined.py <参数JSON文件>")
        print("\n参数示例:")
        print(json.dumps({
            'files': ['test.dxf'],
            '目标高程': '-10.0',
            '设计断面线图层': 'DMX',           # 原始断面线（回淤下边界）
            '更新断面线图层': '20260317',       # 更新断面线（分层算量断面线，回淤上边界）
            '包络线类型': 'lower',
            '区分设计超挖': False,
            '输出目录': ''
    ...
    run_autosection_backfill_combined(params, log_func)
