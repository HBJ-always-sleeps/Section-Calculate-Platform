...
def calculate_basepoint_with_intersection(grid, grid_size, frame: SectionFrame, small_rect_bbox: Tuple[float, float, float, float]) -> Optional[Tuple[float, float]]:
    """计算基点位置：内框中心线与内框顶边交点，再找最近交点
    
    关键改进：使用内框中心线，而不是断面框中心线
    因为断面框会变化，但内框是稳定的参照物
    
    步骤：
    1. 计算基础位置：内框中心线与内框顶边的交点
    2. 在基础位置附近搜索最近的顶点聚类（交点）
    """
    # 使用内框中心线（不是断面框中心线！）
    inner_center_x = (small_rect_bbox[0] + small_rect_bbox[2]) / 2
    top_y = small_rect_bbox[3]  # 内框顶边Y坐标
    
    base_point = (inner_center_x, top_y)
    
    # 找最近的交点
    nearest_intersection = find_nearest_intersection(grid, grid_size, base_point[0], base_point[1], search_radius=15.0)
